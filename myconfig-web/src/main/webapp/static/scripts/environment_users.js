var EnvironmentUsers = function () {
	
	return {
		assignEnvFunction: function (img) {
			var set = img.getAttribute("set");
			var application = img.getAttribute("application");
			var environment = img.getAttribute("environment");
			var user = img.getAttribute("user");
			var fn = img.getAttribute("function");
			var method = (set == "no") ? 'add' : 'remove';
			var url = 'ui/user/' + user + '/application/' + application + '/environment/' + environment + '/function/' + fn + '/' + method; 
